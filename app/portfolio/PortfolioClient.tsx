...
import {
  formatINR, epfProjection, npsProjection, ppfProjection,
...
function buildInitialEditState(profile: Profile, holdings: DBHolding[]): EditState {
  const indianStocks = holdings.filter(
    h => h.category === "indian_stock" && h.notes !== "indian_monthly_inv"
  );
  const usStocks = holdings.filter(
    h => h.category === "us_stock" && h.notes !== "us_monthly_inv"
  );
  const mutualFunds = holdings.filter(
    h => h.category === "mf" && !h.notes?.startsWith("sip") && h.value_inr > 0
  );
  const sips = holdings.filter(
    h => h.category === "mf" && h.notes?.startsWith("sip")
  );
  const physical = holdings.find(
    h => h.category === "gold" && h.name.toLowerCase().includes("physical")
  );
  const goldEtf = holdings.find(
    h => h.category === "gold" && !h.name.toLowerCase().includes("physical")
  );
  ...
  return {
    ...
    mutualFunds: mutualFunds.map(h => ({
      name: h.name, value: String(h.value_inr),
      fundType: h.notes || "index",
    })),
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3 mt-6 first:mt-0">
      {title}
    </div>
  );
}
...
function ProfileCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-surface border border-slate-100 rounded-xl p-3">
...
function EditInput({
  ...
  return (
    <input
      ...
      className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-slate-900 text-sm w-full focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-400/10 bg-white border-slate-200"
    />
  );
}
...
function EditSelect({
  ...
  return (
    <select
      value={value}
      onChange={e => onChange(e.target.value)}
      className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-slate-900 text-sm w-full focus:outline-none focus:border-orange-400"
    >
      {options.map(o => (
...
export default function PortfolioClient({
  ...
  const [editState, setEditState] = useState<EditState>(() =>
    profile ? buildInitialEditState(profile, holdings) : buildInitialEditState({
      id: "", full_name: "", age: null, fire_target_age: null,
      monthly_income: null, monthly_expense: null, parent_support: null,
      tax_bracket: 30, tax_regime: "new", fire_monthly_expense: null, risk_score: 5,
    }, [])
  );
  ...
  async function savePortfolio() {
    ...
    try {
      ...
      // Validate
      for (const s of editState.indianStocks) {
        if (s.name && n(s.value) < 0) throw new Error("Values cannot be negative");
      }
      ...
      editState.indianStocks
        .filter(s => s.name && n(s.value) > 0)
      ...
      editState.mutualFunds
        .filter(s => s.name && n(s.value) > 0)
        .forEach(s => newHoldings.push({
          category: "mf", name: s.name, value_inr: n(s.value), notes: s.fundType,
        }));

      if (n(editState.physicalGold) > 0)
        newHoldings.push({ category: "gold", name: "Physical gold", value_inr: n(editState.physicalGold) });
      if (n(editState.goldEtf) > 0)
        newHoldings.push({ category: "gold", name: "Gold ETF/SGB", value_inr: n(editState.goldEtf) });
      if (n(editState.fd) > 0)
        newHoldings.push({ category: "fd", name: "FD/Emergency", value_inr: n(editState.fd) });

      if (n(editState.epf.value) > 0)
        newHoldings.push({
          category: "epf", name: "EPF",
          value_inr: n(editState.epf.value),
      ...
      if (n(editState.nps.value) > 0 || n(editState.nps.monthly) > 0)
        newHoldings.push({
          category: "nps", name: "NPS",
          value_inr: n(editState.nps.value),
          monthly_contribution: n(editState.nps.monthly),
          notes: `alloc=${editState.nps.alloc}`,
        });
      if (n(editState.ppf.value) > 0 || n(editState.ppf.monthly) > 0)
        newHoldings.push({
          category: "ppf", name: "PPF",
          value_inr: n(editState.ppf.value),
          monthly_contribution: n(editState.ppf.monthly),
          notes: `years_to_maturity=${editState.ppf.years}`,
        });
      if (n(editState.lic.value) > 0)
        newHoldings.push({
          category: "lic", name: "LIC",
          value_inr: n(editState.lic.value),
          monthly_contribution: n(editState.lic.annualPremium) / 12,
          notes: `type=${editState.lic.licType}`,
        });

      editState.sips
        .filter(s => s.name && n(s.monthly) > 0)
        .forEach(s => newHoldings.push({
          category: "mf", name: `SIP: ${s.name}`,
          value_inr: 0, monthly_contribution: n(s.monthly),
          notes: `sip;type=${s.sipType}`,
        }));

      if (n(editState.usMonthly) > 0)
        newHoldings.push({
          category: "us_stock", name: "US Monthly Investment",
          value_inr: 0, monthly_contribution: n(editState.usMonthly),
          notes: "us_monthly_inv",
        });
      if (n(editState.indianMonthly) > 0)
        newHoldings.push({
          category: "indian_stock", name: "Indian Monthly Investment",
          value_inr: 0, monthly_contribution: n(editState.indianMonthly),
          notes: "indian_monthly_inv",
        });

      // Create snapshot (handles holdings insert)
  ...
  if (!profile || holdings.length === 0 && !profile) {
    return (
      <div className="card text-center py-12">
        <div className="text-4xl mb-4">📊</div>
        <h2 className="text-xl font-semibold text-ink mb-2">No portfolio data yet</h2>
        <p className="text-sm text-slate-500 mb-6">
          Complete onboarding to see your portfolio.
        </p>
        <a href="/onboarding" className="btn-primary px-8">Set up portfolio →</a>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {isEditing ? (
        <EditView state={editState} setState={setEditState} />
      ) : (
        <ViewMode profile={profile} holdings={holdings} />
      )}
...
function ViewMode({ profile, holdings }: { profile: Profile; holdings: DBHolding[] }) {
  ...
  const epfProjected = epfH
    ? epfProjection(epfH.value_inr, epfYour, epfEmployer, yearsToFire)
    : 0;
  const npsResult = npsH
    ? npsProjection(npsH.value_inr, npsH.monthly_contribution || 0, yearsToFire)
    : null;
  const ppfProjected = ppfH
    ? ppfProjection(ppfH.value_inr, ppfH.monthly_contribution || 0, Math.min(ppfYears, yearsToFire))
    : 0;
    ...
    <div className="space-y-5">
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          <ProfileCard label="Age" value={`${profile.age ?? "—"} years`} />
          <ProfileCard label="Target retirement age" value={`${profile.fire_target_age ?? "—"} years`} />
          <ProfileCard label="Monthly income" value={profile.monthly_income ? formatINR(profile.monthly_income) : "—"} />
          <ProfileCard label="Monthly expense" value={profile.monthly_expense ? formatINR(profile.monthly_expense) : "—"} />
          <ProfileCard label="Parent support" value={profile.parent_support ? `${formatINR(profile.parent_support)}/mo` : "—"} />
          <ProfileCard label="Tax regime" value={profile.tax_regime === "new" ? "New regime" : "Old regime"} />
          <ProfileCard label="Risk score" value={profile.risk_score ? `${profile.risk_score} / 10` : "—"} />
        </div>
      </div>

      {/* Section 2 — Liquid investments */}
      <div className="card">
        <SectionHeader title="Liquid investments" />

        {indianStocks.length > 0 && (
          <div className="mb-5">
            <div className="text-xs font-medium text-slate-500 mb-2">Indian stocks</div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[400px]">
                <thead>
                  <tr className="text-xs text-slate-400 border-b border-slate-100">
                    <th className="text-left py-2 pr-4 font-medium">Name</th>
                    <th className="text-right py-2 pr-4 font-medium">Current value</th>
                    <th className="text-right py-2 font-medium">% of liquid</th>
                  </tr>
                </thead>
                <tbody>
                  {indianStocks.map((h, i) => (
                    <tr key={h.id || i} className="border-b border-slate-50 last:border-0">
                      <td className="py-2.5 pr-4 font-medium text-ink">{h.name}</td>
                      <td className="py-2.5 pr-4 text-right text-ink">{formatINR(h.value_inr)}</td>
        ...
        {usStocks.length > 0 && (
          <div className="mb-5">
            <div className="text-xs font-medium text-slate-500 mb-2">US stocks</div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[400px]">
                <thead>
                  <tr className="text-xs text-slate-400 border-b border-slate-100">
                    <th className="text-left py-2 pr-4 font-medium">Name</th>
                    <th className="text-right py-2 pr-4 font-medium">Current value (INR)</th>
                    <th className="text-right py-2 font-medium">% of liquid</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {mfs.length > 0 && (
          <div className="mb-5">
            <div className="text-xs font-medium text-slate-500 mb-2">Mutual funds</div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[400px]">
                <thead>
                  <tr className="text-xs text-slate-400 border-b border-slate-100">
                ...
                <tbody>
                  {mfs.map((h, i) => (
                    <tr key={h.id || i} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="py-2.5 text-right text-ink">{formatINR(h.value_inr)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div>
          <div className="text-xs font-medium text-slate-500 mb-2">Gold &amp; cash</div>
          <div className="space-y-2">
            <ViewRow label="Physical gold" value={goldPhysical ? formatINR(goldPhysical.value_inr) : "—"} />
            <ViewRow label="Gold ETF / SGB" value={goldEtf ? formatINR(goldEtf.value_inr) : "—"} />
            <ViewRow label="FD / Emergency fund" value={fd ? formatINR(fd.value_inr) : "—"} />
          </div>
        </div>

        {totalLiquid === 0 && (
          <p className="text-sm text-slate-400 mt-3">No liquid investments added yet.</p>
        )}
      </div>

      {/* Section 3 — Locked investments */}
      <div className="card">
        <SectionHeader title="Locked investments" />
        <div className="grid sm:grid-cols-2 gap-4">
          <LockedCard
            ...
            rows={[
              ...
              { label: `Projected at age ${profile.fire_target_age || 45}`, value: epfProjected > 0 ? formatINR(epfProjected) : "—", highlight: true },
            ]}
          />
          <LockedCard
            title="NPS"
            ...
            rows={[
              ...
              { label: `Projected at age ${profile.fire_target_age || 45}`, value: npsResult ? formatINR(npsResult.atRetirement) : "—", highlight: true },
              { label: "Projected at age 60", value: npsResult ? formatINR(npsResult.at60) : "—", highlight: true },
            ]}
          />
          <LockedCard
            title="PPF"
            ...
            rows={[
              ...
              { label: "Projected at maturity", value: ppfProjected > 0 ? formatINR(ppfProjected) : "—", highlight: true },
            ]}
          />
          <LockedCard
            title="LIC / Insurance"
      ...
      {/* Section 4 — Monthly investments */}
      <div className="card">
        <SectionHeader title="Monthly investments" />
        <div className="overflow-x-auto">
            ...
            <tbody>
              {sips.map((h, i) => (
                <tr key={h.id || i} className="border-b border-slate-50">
                  ...
                  <td className="py-2.5 pr-4 text-slate-500">
                    {SIP_TYPE_LABELS[parseNotes(h.notes, "type")] || parseNotes(h.notes, "type") || "SIP"}
                  </td>
              ...
              <tr>
                <td className="py-2.5 pr-4 font-bold text-ink" colSpan={2}>Total monthly investment</td>
                <td className="py-2.5 text-right font-bold text-ink">{formatINR(totalMonthly)}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <ProfileCard
            label="Post-retirement monthly expense"
            value={profile.fire_monthly_expense ? formatINR(profile.fire_monthly_expense) + "/mo" : "—"}
          />
          <ProfileCard label="Risk appetite score" value={profile.risk_score ? `${profile.risk_score} / 10` : "—"} />
          <ProfileCard label="Property owned" value="—" />
          <ProfileCard label="Dependents" value="—" />
        </div>
      </div>
    </div>
  );
...
function LockedCard({ title, present, rows }: {
  ...
  return (
    <div className={`rounded-xl border p-4 ${present ? "border-slate-100 bg-white" : "border-slate-100 bg-surface"}`}>
      <div className="font-semibold text-ink text-sm mb-3">{title}</div>
      {!present && (
        ...
        <div className="space-y-2">
          {rows.map(row => (
            <div key={row.label} className="flex justify-between text-xs">
              ...
              <span className={row.highlight ? "font-semibold text-orange-600" : "text-ink"}>{row.value}</span>
            </div>
          ))}
        </div>
      )}
...
function EditView({ state, setState }: {
  state: EditState;
  ...
  function blankStock(): StockRow { return { name: "", value: "" }; }
  function blankMF(): MFRow { return { name: "", value: "", fundType: "index" }; }
  function blankSIP(): SIPRow { return { name: "", monthly: "", sipType: "index" }; }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Full name">
            <EditInput value={state.profile.full_name} onChange={v => setProfile({ full_name: v })} placeholder="Your name" />
          </Field>
          <Field label="Age">
            <EditInput value={state.profile.age} onChange={v => setProfile({ age: v })} type="number" placeholder="30" />
          </Field>
          <Field label="Target retirement age">
            <EditInput value={state.profile.fire_target_age} onChange={v => setProfile({ fire_target_age: v })} type="number" placeholder="45" />
          </Field>
          <Field label="Monthly income (₹)">
            <EditInput value={state.profile.monthly_income} onChange={v => setProfile({ monthly_income: v })} type="number" placeholder="200000" />
          </Field>
          <Field label="Monthly expense (₹)">
            <EditInput value={state.profile.monthly_expense} onChange={v => setProfile({ monthly_expense: v })} type="number" placeholder="100000" />
          </Field>
          <Field label="Parent support/mo (₹)">
            <EditInput value={state.profile.parent_support} onChange={v => setProfile({ parent_support: v })} type="number" placeholder="0" />
          </Field>
          <Field label="Tax bracket (%)">
            <EditSelect
              value={state.profile.tax_bracket}
              onChange={v => setProfile({ tax_bracket: v })}
              options={[{ value: "5", label: "5%" }, { value: "10", label: "10%" }, { value: "15", label: "15%" }, { value: "20", label: "20%" }, { value: "30", label: "30%" }]}
            />
          </Field>
          <Field label="Tax regime">
            <EditSelect
              value={state.profile.tax_regime}
              onChange={v => setProfile({ tax_regime: v })}
              options={[{ value: "new", label: "New regime" }, { value: "old", label: "Old regime" }]}
            />
          </Field>
          <Field label="Risk score (1–10)">
            <EditInput value={state.profile.risk_score} onChange={v => setProfile({ risk_score: v })} type="number" placeholder="5" />
          </Field>
          <Field label="Post-retirement monthly expense (₹)">
            <EditInput value={state.profile.fire_monthly_expense} onChange={v => setProfile({ fire_monthly_expense: v })} type="number" placeholder="80000" />
          </Field>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <EditSubSection title="Indian stocks">
          ...
          <button className="text-xs text-orange-500 hover:text-orange-600 font-medium mt-1" onClick={() => setState(s => ({ ...s, indianStocks: [...s.indianStocks, blankStock()] }))}>+ Add stock</button>
        </EditSubSection>

        <EditSubSection title="US stocks">
          ...
          <button className="text-xs text-orange-500 hover:text-orange-600 font-medium mt-1" onClick={() => setState(s => ({ ...s, usStocks: [...s.usStocks, blankStock()] }))}>+ Add stock</button>
        </EditSubSection>

        <EditSubSection title="Mutual funds">
          {state.mutualFunds.map((row, i) => (
            <RowEditor key={i}
              ...
              middle={
                <EditSelect value={row.fundType} onChange={v => setState(s => ({ ...s, mutualFunds: s.mutualFunds.map((r, j) => j === i ? { ...r, fundType: v } : r) }))}
          ...
          <button className="text-xs text-orange-500 hover:text-orange-600 font-medium mt-1" onClick={() => setState(s => ({ ...s, mutualFunds: [...s.mutualFunds, blankMF()] }))}>+ Add fund</button>
        </EditSubSection>

        <EditSubSection title="Gold &amp; cash">
      ...
      <div className="card">
        ...
        <EditSubSection title="LIC / Insurance">
          <div className="grid sm:grid-cols-3 gap-3">
      ...
      {/* Section 4 — Monthly SIPs */}
      <div className="card">
        <SectionHeader title="Monthly investments" />

        <EditSubSection title="SIPs">
          {state.sips.map((row, i) => (
            <RowEditor key={i}
              ...
              middle={
                <EditSelect value={row.sipType} onChange={v => setState(s => ({ ...s, sips: s.sips.map((r, j) => j === i ? { ...r, sipType: v } : r) }))}
                  options={[{ value: "index", label: "Index" }, { value: "active", label: "Active" }, { value: "elss", label: "ELSS" }, { value: "debt", label: "Debt" }]}
                />
              }
        ...
        <EditSubSection title="Direct stock investments (monthly)">
...
function EditSubSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
...
function RowEditor({
  ...
  return (
    <div className={`grid gap-2 mb-2 items-start ${middle ? "grid-cols-[1fr_auto_auto_auto]" : "grid-cols-[1fr_auto_auto]"}`}>
      {left}
      {middle && <div className="min-w-[130px]">{middle}</div>}
      <div className="min-w-[130px]">{right}</div>
      <button
        onClick={onDelete}
        className="mt-0.5 p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"
        title="Remove"
      >
